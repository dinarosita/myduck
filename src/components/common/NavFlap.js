...
export default function NavFlap(props) {
  ...
  return (
    <div className={`
        ...
        absolute z-10 bg-gradient-to-r from-white via-white to-sol-25 w-80 
        ${flapOpen ? "left-0" : "-left-80"}
        xs:
    `}>
        <IonicButton ionic="close" onClick={handleCloseFlap} addClass="absolute top-2 right-3"></IonicButton>
      {props.children}
    </div>
  );
}
